...
def print_array_helper(name, arr):
    ...
    for k in range(16):
        ...
        if k == 15:
            print("    ", ", ".join("0x{0:02X}".format(k) for k in arr[n:n+16]))
        else:
            print("    ", ", ".join("0x{0:02X}".format(k) for k in arr[n:n+16]), end=",\n")
...
def gf_2n_mul_faster(a,b):
    ...
    # Why 255 but not 256?
    # Because expo_table[255] == expo_table[0] == 1, the cycle ends at [254] (the 255th element)
    # Don't forget we can't get 0 by muliplication
...
def generate_gf_2n_expo_table_of_n(n):
    result = [None] * 256
    ...
    # 1) No way to get 0, so there are 255 numbers in multiplication
    # 2) So the cycle ends at 254. table[255] == table[0]

    for i in range(1,256):
        result[i] = gf_2n_mul_faster(result[i-1], n)

    print_array_helper("gf_2n_expo_table_of_{}".format(n), result)
    return result

def generate_gf_2n_expo_table_of_3():
    return generate_gf_2n_expo_table_of_n(3)
...
def test_mix_columns_operation():
    ...
    # wrong case
    assert mix_columns_operation(0xed, 0x1b, 0x45, 0x74) != (0x4d, 0x7e, 0xbd, 0xf8)

def create_rcon():
    # rcon[i] is 2^(i-1) in GF(2^n)
    # In other words, an expo 2 table with shifted result

    result = [None] * 256
    result[1:256] = generate_gf_2n_expo_table_of_n(2)[0:255]
    result[0] = result[255]

    print_array_helper("rcon", result)
    return result
